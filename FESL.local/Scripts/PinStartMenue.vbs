'* VBS-Script zum Anheften von Verknüpfungen an das Startmenü
'* für Windows 7 und Windows Server 2008 R2 (Deutsch und Englisch)
'* (abgeleitet von VBS-Script "PinTaskBar.vbs")
'* (c) 2015 by bent.schrader@googlemail.com
'* Version 1.0

'* ergänzt und erweitert von WCS am 25.09.2015
'* – bevor alte Verknüpfungen aus dem (TargetFolder) gelsöcht werden, wird überprüft,
'*   ob sie entweder im Startmenü oder in der Taskleiste noch angeheftet sind
'* Version 1.3

'* WCS am 28.06.2016
'* – Funktion auf Windows 8 und Windows 10 erweitert
'* Version 1.4
...
If objArgs.Count > 0 Then
	...
	' Erstellen und aktualisieren von StartMenü-Verknüpfungen
	' ———————————————————
	...
	For Each File In objSourceFolder.Files
		...
		If LCase(objFSO.GetExtensionName(objSourceFile)) = "lnk" Then
			...
			If objFSO.FileExists(strTargetFolder & File.Name) Then
				...
				If objSourceFile.DateLastModified <> objTargetFile.DateLastModified Then
					...
					For Each Itemverb In objLNKItem.Verbs
						' Wenn der Vergleich positiv ausgefallen ist, werden die Verknüpfungen vom StartMenü gelöscht (DEU) (W7)
						If Replace(Itemverb.Name, "&", "") = "Vom Startmenü lösen" Then Itemverb.DoIt
						' Wenn der Vergleich positiv ausgefallen ist, werden die Verknüpfungen vom StartMenü gelöscht (DEU) (W8, W10)
						If Replace(Itemverb.Name, "&", "") = "Von ""Start"" lösen" Then Itemverb.DoIt
						' Wenn der Vergleich positiv ausgefallen ist, werden die Verknüpfungen vom Startmenü gelöscht (ENG) (W7)
			...
			For Each Itemverb In objLNKItem.Verbs
				' Verknüpfung wird (wieder) angeheftet (DEU) (W7)
				If Replace(Itemverb.Name, "&", "") = "An Startmenü anheften" Then Itemverb.DoIt
				' Verknüpfung wird (wieder) angeheftet (DEU) (W8, W10)
				If Replace(Itemverb.Name, "&", "") = "An ""Start"" anheften" Then Itemverb.DoIt
				' Verknüpfung wird (wieder) angeheftet (ENG) (W7)
